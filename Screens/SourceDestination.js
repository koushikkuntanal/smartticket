...
const SourceDestination = ({route}) =>{
  ...
  let orderId = 'Kou111000',customerId = 'kou50';
  ...
  useEffect(()=>{
   ...
   (async()=>{
    await getAssetIdApiForEmp({
      "id":assestdata
    }).then(async res=>{console.log('response when getAssetIdApiForEmp() is hit ')
       
       {console.log('getting rev rouet flag with asset id',res.data.AstId)
       await getRevRouteFlagApi({
      
         "AstId" : res.data.AstId
       }).then(res=>{console.log('revRoute Flag',res.data)
       setLoading(true);
         setRevData(res.data[(res.data).length-1].revRoute);
         setLoading(false);
       }).catch(err=>{console.log('err whne revRoute is get',err)
       alert(err);
})}
        await getRouteIdApi({
...
//   }
  ...
  const getFareTo=async(toV)=>{
    setLoading(true);
    console.log('from and to',from,toV);
    await getFareForUsers({
        "from":from,
        "to":toV,
    }).then(res=>{console.log('data when get fare is hit',res.data.Fare)
 ...
 {/* {console.log('rev stages',reversedStages)} */}
   ...
   <View>
    ...
    <View style={styles.form}>
      ...
      <Text style={{justifyContent:'center',alignItems:'center',flex:1}}>To</Text>
       ...
       <Picker
             // itemStyle={{height:40}}
              selectedValue={to}
              onValueChange={async (value, index) => {setTo(value);
                getFareTo(value);
    ...
    {/* {console.log(apiFare)} */}
    {(apiFare != undefined)?<View style={styles.fare}><Text style={styles.textfare}>Amount : {'\u20B9'} {apiFare*passengerNumber}</Text></View> : null}
